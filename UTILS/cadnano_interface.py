...
def parse_helix (string):
    ...
    ret.skiploop_bases = len(ret.skip) + sum(ret.loop) - sum(ret.skip) #TODO find why they have skip
...
def main():
    ...
    for h in cadsys.vhelices:
        """
        # deal with skips:
        #TODO need to do something to change h.scaf[i] itself
        for i, skip_note in enumerate(h.skip):
            if skip_note == 1:
                h.scaf[i-1].V_1 = h.scaf[i].V_1
                h.scaf[i-1].b_1 = h.scaf[i].b_1
                h.scaf[i+1].V_0 = h.scaf[i].V_0
                h.scaf[i+1].b_0 = h.scaf[i].b_0
                h.stap[i-1].V_1 = h.stap[i].V_1
                h.stap[i-1].b_1 = h.stap[i].b_1
                h.stap[i+1].V_0 = h.stap[i].V_0
                h.stap[i+1].b_0 = h.stap[i].b_0
                """
                ...
                if s.V_1 == -1 and s.b_1 == -1:
                    ...
                    if abs(s.b_1 - s.b_0) == 2:
                        ...
                        for j in range(len(partner_list_scaf)):
                            if [h.num, column] == partner_list_scaf[j]: # what?
