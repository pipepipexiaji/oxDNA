...
stmp = l.get_system()
nconf, nblock = 0, 0
msd = np.zeros(5000, dtype = np.float64)
nmsd = np.zeros(5000, dtype = np.int64)
...
out = open ("msd.dat", "w")
for i in range(1, nconf):
    print >> out, "%7i %10.5f" % (i, msd[i] / nmsd[i])
out.close ()
...
#         print >> out, "%7i %20.5g" % (i, msd[i] / nmsd[i])
